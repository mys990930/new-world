...
- `coord`
- block storage
  - uniform single-block fast path
  - dense `BlockId` array payload
...
- empty/all-same chunk는 uniform 표현으로 유지할 수 있고, 첫 heterogeneous write 시 dense 표현으로 materialize된다.

## 공개 인터페이스

```rust
ChunkData::new_empty(coord: ChunkCoord) -> ChunkData
ChunkData::new_filled(coord: ChunkCoord, block: BlockId) -> ChunkData
...
- 논리적으로는 모든 local 좌표가 항상 하나의 `BlockId`를 가진다.
...
- 현재 구현은 empty chunk를 uniform `AIR`로 보관하고, dense block array는 실제로 서로 다른 block이 섞일 때만 사용한다.
- `ChunkSnapshot`도 uniform fast path를 유지하므로 save/load/jobs 전달 시 all-air chunk를 불필요하게 materialize하지 않는다.
