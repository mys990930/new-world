...
pub(super) fn resolve_floodplain_bench(input: FloodplainBenchInput) -> FloodplainBenchResult {
    ...
    let bank_shelf_shell =
        band_influence(input.distance_blocks, bank_shelf_center, bank_shelf_width)
            * input.bank_influence.max(input.floodplain_influence * 0.72)
    ...
    let flood_bench_shell =
        band_influence(input.distance_blocks, flood_bench_center, flood_bench_width)
            * input.floodplain_influence.max(input.outer_influence * 0.70)
