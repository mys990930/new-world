...
pub(super) fn apply_channel_carve_layers(
    ...
    let layered_height = (smoothed_height - terrain_cut)
        .min(smoothed_height)
        .clamp(MIN_HYDROLOGY_Y, MAX_HYDROLOGY_Y);
    let core_target = core_floor_target
        .min(smoothed_height)
        .clamp(MIN_HYDROLOGY_Y, MAX_HYDROLOGY_Y);
    let core_weight = (core_influence * core_profile_strength).clamp(0.0, 1.0);
    let core_matched_height = super::lerp_f32(layered_height, core_target, core_weight)
        .min(smoothed_height)
        .clamp(MIN_HYDROLOGY_Y, MAX_HYDROLOGY_Y);

    let floodplain_target = (water_surface + floodplain_freeboard)
        .min(smoothed_height)
        .clamp(MIN_HYDROLOGY_Y, MAX_HYDROLOGY_Y);
    let floodplain_weight = ((bank_influence * 0.52 + floodplain_influence * 0.48)
        * (1.0 - core_influence * 0.78)
        * floodplain_profile_strength)
        .clamp(0.0, 1.0);

    if core_matched_height > floodplain_target {
        super::lerp_f32(core_matched_height, floodplain_target, floodplain_weight)
            .min(smoothed_height)
            .clamp(MIN_HYDROLOGY_Y, MAX_HYDROLOGY_Y)
    } else {
        core_matched_height
