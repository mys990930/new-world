...
- Region influence sampling should treat atlas cells as jittered semantic seeds for visible
  transition neighborhoods, not as rectangular material-boundary cells. Hard region ownership may
  still be query-stable, but visible influence must not be born only from axis-aligned cell edges.
