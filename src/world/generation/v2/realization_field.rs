...
use crate::world::atlas::{AtlasCell, CoastalContext, HydrologyContext, RegionClassCell, TerrainFormFamily};
...
use super::{ChunkGenerationV2Inputs, sample_atlas_fields_fractional, sample_region_weights};

pub const REALIZATION_NODE_CHUNK_SPAN: i32 = 2;
pub const REALIZATION_NODE_BLOCK_SPAN: u32 =
    (REALIZATION_NODE_CHUNK_SPAN * CHUNK_EDGE_I32) as u32;
...
impl RealizationSample {
    ...
    fn blend_toward(self, anchor: Self, amount: f32) -> Self {
        ...
        Self {
            ...
            ridge_shoulder_lift: lerp_f32(
                self.ridge_shoulder_lift,
                anchor.ridge_shoulder_lift,
                t,
            ),
            ...
            ridge_preservation: lerp_f32(
                self.ridge_preservation,
                anchor.ridge_preservation,
                t,
            ),
            meso_relief_reserve: lerp_f32(
                self.meso_relief_reserve,
                anchor.meso_relief_reserve,
                t,
            ),
        }
    }
...
fn solve_realization_node(
    ...
    smoothed.blend_toward(center.sample, anchor_pull).finalized()
}
...
fn dominant_region_cell(region_samples: &[super::RegionSampleWeight; 4]) -> RegionClassCell {
...
fn modulate_source_hint(mut sample: RealizationSample, field: AtlasCell) -> RealizationSample {
    let uplift_delta = (field.macro_elevation - 0.5) * 8.0 + (field.continent_core_factor - 0.5) * 3.5;
    ...
    sample.corridor_depth *= 0.88 + field.river_flow_potential * 0.18 + field.riverine_factor * 0.08;
...
fn realization_permeability(a: RealizationSourceHint, b: RealizationSourceHint) -> f32 {
    ...
    let marine_transition =
        a.region.coastal_context == CoastalContext::Marine || b.region.coastal_context == CoastalContext::Marine;
    let inland_transition =
        a.region.coastal_context == CoastalContext::Inland || b.region.coastal_context == CoastalContext::Inland;
...
mod tests {
    use super::*;
    use crate::world::generation::v2::prepare_chunk_v2_inputs;
    ...
    fn realization_field_samples_match_across_neighboring_chunk_contexts() {
        ...
        for offset_z in 0..CHUNK_EDGE_I32 {
            ...
            let left = sample_chunk_realization_field(&left_patch, boundary_world_x as f32 - 0.5, world_z);
            let right = sample_chunk_realization_field(&right_patch, boundary_world_x as f32 + 0.5, world_z);
