...
- a feature-owned helper should read the prototype baseline plus atlas-owned guide context, resolve any shared per-feature world-space objects it needs for the chunk window, and return a target local surface with an explicit blend/falloff contract
...
- `hill_cluster` is the first feature being moved from `generic delta_y` treatment to `feature-owned target-surface resolve`
  ...
  - hill-cluster runtime should first resolve a shared hill-cluster window for the chunk, then sample that same resolved object set per column rather than rebuilding hill guesses column-by-column
