...
- `hill_cluster` is the first feature being moved from `generic delta_y` treatment to `feature-owned target-surface resolve`
  ...
  - hill-source pruning should remain sample-relevance-aware and seam-safe so hill support does not snap across chunk boundaries while moving through one continuous hill mass
