...
pub fn voxelize_chunk(plan: &VoxelizationPlan, registry: &BlockRegistry) -> ChunkData {
    let chunk_min_y = plan.chunk.1 * CHUNK_EDGE_I32;
    ...
    for local_z in 0..CHUNK_EDGE_I32 {
        for local_x in 0..CHUNK_EDGE_I32 {
            ...
            for local_y in 0..CHUNK_EDGE_I32 {
                ...
                } else if column.water_top_y.is_some_and(|water_top_y| world_y <= water_top_y) {
    ...
    ChunkData::from_blocks(plan.chunk, blocks)
}
...
mod tests {
    ...
    ) -> (crate::world::HydrologySolve, ChunkSurfacePlan, VoxelizationPlan, ChunkData) {
    ...
    fn voxelization_plan_is_deterministic() {
        ...
        assert_eq!(a_plan.columns.len(), (CHUNK_EDGE_I32 * CHUNK_EDGE_I32) as usize);
    }
    ...
    fn voxelization_emits_solid_ground_for_dry_columns() {
        ...
        let local_y = u8::try_from(world_y).expect("dry surface should land inside the sampled chunk");
        let local = LocalBlockCoord::new(local_x, local_y, local_z).expect("surface coord should be valid");
    ...
    fn voxelization_places_water_or_ice_above_hydrology_channels() {
        ...
        let local = LocalBlockCoord::new(local_x, local_y, local_z).expect("water coord should be valid");
        let block = chunk_data
            .get_block(local)
