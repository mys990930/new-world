...
- assemble the padded atlas windows that generation scaffolding needs for a target chunk
...
- expose the deterministic generation atlas footprint so multi-chunk tools can reuse identical input bundles
...
- this module now exists only as a shared generation input-assembly helper
...
- this snapped footprint is now available through the generation input API for local preview/job caches; the cache key is the atlas footprint, not the visual chunk coordinate
