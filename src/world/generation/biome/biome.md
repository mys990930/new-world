...
2. `Desert`
   ...
   - 이유: 실제 hot desert는 매우 건조하고 덥고 내륙성이 강하다. 관측 hydration p1이 약 `0.28`이므로
     `0.30`은 드문 극건조 조건이다.
3. `DryShrubland`
   - 기준: `ruggedness >= 0.11`, `hydration < 0.40`, `is_inland_dry_context(0.10)`
   - 이유: 거칠고 건조한 구릉/사면은 연속 초원보다 관목지로 읽힌다. ruggedness는 전체 p95보다 높은
     relief를 잡는다. 이번 중간 강도 조정에서는 `ruggedness >= 0.11`인 건조 cell을 열린 초원으로 두지
     않고 관목지로 보낸다.
4. `SemiDesert`
   - 기준: `hydration < 0.36`, `effective_temperature >= 0.52`, `is_inland_dry_context(0.18)`
   - 이유: desert보다는 덜 극단적이지만 여전히 건조하고 따뜻한 내륙/비해안 지형이다.
5. `Steppe`
   - 기준: `effective_temperature <= 0.53`, `hydration < 0.38`, `is_inland_dry_context(0.27)`,
     `ruggedness < 0.11`
   - 이유: steppe는 비교적 서늘하거나 온난한 대륙성 건조 초원이다. 내륙성, 실제 건조도, 낮은 relief를
     중간 강도로 요구해 연안 또는 약건조 grassland가 steppe로 과다 분류되지 않게 한다.
...
2. `PolarBarrens` 또는 `Tundra`
   - 기준: `effective_temperature <= 0.31`, 또는
     `ruggedness >= 0.30 && effective_temperature <= 0.39 && hydration < 0.39`
...
3. `Tundra`
   - 기준: `effective_temperature <= 0.34`에서는 `hydration < 0.36`이면 `PolarBarrens`, 아니면
     `Tundra`; 추가로 `effective_temperature <= 0.35`는 `Tundra` fallback
   - 이유: 숲이 성립하기 어려운 열린 한랭 지형이다. 이전 `<= 0.36` fallback보다는 좁지만 강한 축소안의
     `<= 0.34` 통합보다는 완화해, `0.35` 안쪽의 한랭 습윤 edge는 tundra로 허용한다.
4. `BorealForest`
   - 기준: `effective_temperature <= 0.43`, `hydration >= 0.50`, `continentality >= 0.13`,
     `mountainness < 0.32`, `coastness <= 0.65`
   - 이유: 차갑고 습한 것만으로 모든 edge를 타이가로 만들지 않는다. boreal forest는 더 선명한
     내륙 냉습 저산지에 가깝게 제한하되, 강한 축소안보다 완화된 coast 제한을 남겨 해양성 cold forest의
     과다 흡수만 줄인다. 산악성이 큰 cell은 alpine/subalpine 쪽 판정에 맡긴다.
5. `Steppe`
   - 기준: `effective_temperature <= 0.43`, `hydration < 0.38`, `continentality >= 0.22`,
     `coastness <= 0.40`, `ruggedness < 0.11`
...
- `BorealForest`, `Tundra`, `Steppe`가 과다해 보일 때 가장 먼저 볼 조건은 cold branch의
  `BorealForest <= 0.43`, `Tundra <= 0.35`, cold `Steppe <= 0.43`, dry branch의 `Steppe <= 0.53`와
  `hydration < 0.38`, `is_inland_dry_context(0.27)`, `ruggedness < 0.11`이다.
- 이번 조정은 직전 완화안과 강한 축소안 사이의 중간 강도 조정이다. `BorealForest`는
  inland/cold/humid/low-mountain에 더해 느슨한 coast 제한을 남기고, `Tundra`는 `<= 0.35` fallback으로
  일부 한랭 edge를 되돌리며, `Steppe`는 dry + inland + smooth 조건을 유지하되 강한 축소안보다 약간
  넓힌다.
