...
`macro_field`는 graph-first generator의 9단계인 Voronoi-derived macro field cache를 소유한다.

이 모듈은 새 지형 의미를 다시 noise로 생성하지 않는다. source of truth는 앞 단계의
`VoronoiGraphPatch`, `GraphMacroMap`, `GraphHydrologyGraph`, final cell context, `BoundaryCache`이며, `macro_field`는
...
- noisy boundary 이후의 canonical curve geometry를 읽어 ridge/coast distance envelope를 계산하고,
  river는 hydrology selected segment별 canonical noisy curve shape를 가능한 유지한 채 tile 단위
  influence pass로 굽는다. 단순 downstream join에는 짧은 transition patch를 추가할 수 있다.
...
2. ridge, coast, selected river guide를 tile-local influence field로 rasterize한다.
   ...
   - selected river는 hydrology의 selected edge topology를 유지하고, 각 segment 내부는 해당
     `NoisyBoundaryCurve.points`를 downstream 방향으로 정렬해 그대로 source shape로 사용한다.
   - macro_field는 river 전체 centerline을 다시 spline으로 해석하지 않는다. segment A와 B가
     `incoming = 1, outgoing = 1`인 단순 downstream node에서 만날 때만 node 주변 제한 반경 안에
     짧은 Bezier transition patch를 추가한다.
...
3. 먼저 nearest macro site를 찾되, sample point가 canonical noisy boundary curve의 blend radius 안에
   ...
   - 이 단계의 visible ownership/mask boundary는 straight nearest-site 선이 아니라 stage 8
...
7. hydrology selected river segment의 topology와 canonical noisy curve를 읽어 anti-aliased thick
   polyline coverage, distance, selected/display flow로 river valley field를 만든다.
   - river 전용 noisy curve는 만들지 않는다.
   - lake boundary/internal/adjacent edge는 hydrology stage에서 selected river가 이미 금지한다.
   - branch/confluence에서는 독립 branch를 잘못 이어 붙이지 않는다. transition patch는 downstream으로
     `incoming = 1, outgoing = 1`인 단순 join에서만 생성하고, node degree가 바뀌면 기존 segment shape를
     그대로 유지한다.
   - join patch는 양쪽 segment의 node 인접 짧은 구간만 잘라낸 뒤 cubic Bezier bridge로 연결한다.
     patch 길이는 local river width와 양쪽 curve 길이에 의해 제한되며, segment 내부 noisy curve는
     whole-chain smoothing이나 corner-only spline으로 재해석하지 않는다.
   - bend angle이 큰 join patch 주변은 effective width를 줄인다. 이 curvature-aware width는 하류
     폭/flat-bed 정책을 없애는 것이 아니라, sharp join에서 capsule union이 만드는 round bulge를
     억제하기 위한 local scale이다.
...
9. 아래 계열로 combined macro height를 계산한다. 이 단계의 river carve는 최종 물/복셀 carve가
...
hydrology segment별 noisy curve를 anti-aliased oriented stroke로 굽는다. 단순 downstream join은
짧은 Bezier transition patch source로 추가된다. ownership과 macro elevation의 noisy-boundary side/blend 판정은 아직 per-sample query로 남아
...
- ridge influence
- river valley strength/distance/flow hint. selected hydrology edge path의 topology와 segment별
  canonical noisy curve shape를 따르되, 단순 join에서는 짧은 transition patch가 외곽을 이어야 한다.
  상류는 좁고 얕지만
...
- final cell context / biome influence. stage 7에서 resolve된 final temperature, hydration,
...
terrain tile 경계를 확인한다고 말할 때의 1차 의미는 macro-field cache grid가 아니라, stage 8의
...
1. `macro_field`는 graph/macro/hydrology/final-cell-context/boundary를 대체하는 source of truth가 아니다.
...
5. river topology는 selected edge id graph를 따른다. macro_field의 carve geometry는 segment 내부의
   canonical noisy polyline을 가능한 유지하고, 단순 downstream join에만 제한된 transition patch를
   추가한다. river valley width, flat-bed radius, carve depth는 selected/display flow에 비례해야 하며,
   고정 폭 corridor를 모든 강에 적용하면 안 된다. valley profile은 center flat-bottom과 shoulder
   falloff를 분리해야 하며, 하류일수록 flat bed가 넓고 완만하게 보여야 한다. bend/corner에서 segment
   capsule union이 만드는 round blob이나 join이 뚝 끊긴 외곽이 보이면 회귀다.
...
11. contour segment는 preview/debug layer이며, source graph/macro/hydrology/final-cell-context/boundary나 heightfield
...
- ridge/coast influence는 selected guide edge의 canonical noisy curve를 tile source pixel로 rasterize한
  뒤 chamfer distance field로 만든다. river influence는 selected edge id topology와 segment별
  canonical noisy curve를 anti-aliased thick polyline으로 굽고, 단순 downstream join에는 짧은 Bezier
  transition patch를 추가한다. subpixel coverage 기반 valley strength와 nearest-segment distance,
  selected/display flow hint를 함께 저장한다. 이로써 강줄기가 점 splat이나 segment endpoint cap의
  원형 흔적으로 보이는 문제를 줄이면서도 sample마다 전체 curve 후보를 반복 탐색하지 않는다.
