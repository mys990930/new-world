...
Ocean/lake mouth bed hints do not cut terrain in this layer. An above-sea ocean-owned sample can keep river diagnostics, but crossing the river threshold by itself must not cut that column into a below-sea river mouth trench. Explicit `estuary_water_strength` may keep estuary fan bed context at the edge or restore water-filled core at the fan center only when `macro_field` has already baked a lowered mouth bed into `combined_macro_height`. Estuary-dominant water uses the separate fan water-depth hint and a small sea-level convergence ceiling instead of the ordinary upstream river fill-depth so the mouth surface does not start high above the baked bed.
