...
Ocean/lake mouth bed hints do not cut terrain in this layer. An above-sea ocean-owned sample can keep river diagnostics, but crossing the river threshold by itself must not cut that column into a below-sea river mouth trench. Explicit `estuary_water_strength` may keep estuary fan bed context at the edge or restore water-filled core at the fan center; it still only reads the already-baked `combined_macro_height`.
