...
- evaluate carried corridors through the shared `corridors::sample_corridor_axis(...)` distance field instead of direct raw segment projection
...
- forwards realization-derived material support axes so later smoothing and surface resolve can
  choose categorical material domains from terrain evidence rather than hard atlas ownership alone
...
- corridor segment endpoints and centers may lie outside the strict chunk bounds, but the influence on prototype columns is still evaluated in chunk-local block space through the shared branch-continuous corridor axis
...
- current implementation note:
  ...
  - the semantic source feeding that realization sample now comes from displaced region-influence sampling rather than axis-aligned four-corner atlas bilerp, so prototype no longer inherits a different boundary basis than surface/material resolve
...
9. forward material support axes with the column so meso and smoothing can refine, not recreate, the
   surface-material evidence
10. emit one `PrototypeColumn` per chunk column and return the completed `BaseHeightfieldPrototype`

## Corridor Influence Rules

- valley seats
  - lower the broad target height along the carried corridor axis, not the raw segment chord
...
- prototype must not turn atlas-cell boundaries, structure segment chords, or repeated detail carriers into large visible straight lines, rectangles, or smooth artificial arcs. Broad guide geometry should be converted into terrain-following seats, shoulders, outlets, and relief envelopes with warped or softened boundaries before later stages see it.
...
7. material and block decisions are deferred, but broad material support axes may be forwarded as
   generation-owned evidence for those later decisions
...
10. any visible long line or curve emitted by prototype must correspond to a natural macro form such as a ridge shoulder, coast, valley, or plateau rim and must not reveal the raw guide primitive that produced it
