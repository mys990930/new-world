...
- mainstem과 explicit tributary 후보가 hydration coherence나 parallel drainage 때문에 매우 가까운
  source/초기 path로 몰리면, score 순서로 가장 좋은 후보를 먼저 수락하고 이후 후보를 world-block
  spacing 기준으로 억제한다.
- explicit tributary끼리 같은 local merge neighborhood로 몰리는 경우에도 후순위 후보를 억제한다.
...
- accepted selected source/path spacing suppression helpers
