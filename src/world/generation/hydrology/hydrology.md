...
- coast-reaching selected river는 land/coast terminal endpoint에서 종료한다. hydrology topology 단계는
  non-lake ocean-owned edge를 추가 selected segment로 연장하지 않는다. 하구 표현이 필요하면 selected
  hydrology topology를 바꾸지 않는 downstream heightfield-local 정책으로 다룬다.
