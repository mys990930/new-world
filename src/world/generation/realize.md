...
- Ground relief now also receives Wave 1A meso hill, basin, escarpment, and terrace bias before final smoothing, so several-chunk terrain identity does not have to come only from profile-local noise.
- Near shore, coast classification now uses warped low-frequency boundary noise and an emergent-shore rule, so shelf/land transitions do not stay locked to a perfectly smooth atlas bilerp contour.
...
- Atlas structure guides now feed the same carve, so explicit nearby river paths dominate centerline placement and only add bounded branch-aligned meander instead of relying on free per-column meander noise.
- Inland river water surfaces now come from a branch-consistent graded waterline and are quantized upward, so shallow water is less likely to disappear or oscillate independently from the carved bed.
...
- `realize` still consumes meso-derived broad relief rather than owning meso generation itself.
