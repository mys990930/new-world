...
mod tests {
    ...
    use crate::world::generation::{
        ...
        build_chunk_smoothed_prototype, prepare_chunk_generation_inputs,
    };
    ...
    fn build_hydrology(
        ...
        let prototype =
            build_chunk_base_heightfield_prototype(chunk, &inputs, &realization, &corridor_window);
        ...
        for seed in seed_chunks {
            for offset_z in -2..=2 {
                for offset_x in -2..=2 {
                    ...
                    let (inputs, corridor_window, smoothed, hydrology) =
                        build_hydrology(chunk, meta);
    ...
    fn seam_has_shared_visible_water(
        ...
        for row in 0..CHUNK_EDGE_I32 as usize {
            ...
            if let (Some(left_water), Some(right_water)) = (
                left_column.water_surface_height,
                right_column.water_surface_height,
            ) {
                let water_delta = (left_water - right_water).abs();
                let terrain_delta =
                    (left_column.terrain_height - right_column.terrain_height).abs();
        ...
        for seed in seed_chunks {
            for offset_z in -2..=2 {
                for offset_x in -2..=2 {
                    ...
                    if seam_has_shared_visible_water(
                        &base_hydrology,
                        &east_hydrology,
                        SharedAxis::X,
                    ) {
    ...
    #[test]
    #[ignore = "slow current generation hydrology pipeline smoke test"]
    ...
    #[test]
    #[ignore = "slow current generation hydrology regression window"]
    ...
    #[test]
    #[ignore = "slow current generation hydrology search smoke test"]
    ...
    #[test]
    #[ignore = "slow current generation hydrology seam smoke test"]
    ...
    #[test]
    #[ignore = "slow target-window water edge support regression"]
    fn target_preview_water_edges_stay_within_bank_support() {
        let meta = WorldMeta::new(42);
        let (_, _, _, hydrology) = build_hydrology(ChunkCoord(-72, 0, 90), &meta);
        let mut water_columns = 0usize;
        let mut unsupported_edges = 0usize;
        let mut max_float_excess = f32::MIN;
        let mut max_block_excess = i32::MIN;

        for local_z in 0..CHUNK_EDGE_I32 {
            for local_x in 0..CHUNK_EDGE_I32 {
                let index = column_index(local_x, local_z);
                let column = hydrology.columns[index];
                let Some(water) = column.water_surface_height else {
                    continue;
                };
                water_columns += 1;
                ...
                for (offset_x, offset_z) in [(1, 0), (-1, 0), (0, 1), (0, -1)] {
                    ...
                    let neighbor = hydrology.columns[column_index(neighbor_x, neighbor_z)];
                    if neighbor.water_surface_height.is_some() {
                        continue;
                    }

                    let float_excess = water - neighbor.terrain_height;
                    let block_excess = water.ceil() as i32 - neighbor.terrain_height.floor() as i32;
                    max_float_excess = max_float_excess.max(float_excess);
                    max_block_excess = max_block_excess.max(block_excess);
                    if float_excess > MIN_VISIBLE_WATER_DEPTH {
                        unsupported_edges += 1;
                    }
                }
            }
        ...
        assert!(water_columns >= 8);
        assert_eq!(unsupported_edges, 0);
        assert!(max_float_excess <= 0.05);
        assert!(max_block_excess <= 1);
    }

    #[test]
    #[ignore = "slow target-window weak shoreline pillar regression"]
    fn target_preview_has_no_high_weak_shoreline_columns() {
        let meta = WorldMeta::new(42);
        let (_, _, _, hydrology) = build_hydrology(ChunkCoord(-72, 0, 90), &meta);
        let mut high_shoreline_columns = 0usize;
        ...
        for local_z in 0..CHUNK_EDGE_I32 {
            for local_x in 0..CHUNK_EDGE_I32 {
                ...
                let Some((water_height, water_distance)) = nearest_visible_water_height_within(
                    &hydrology,
                    local_x,
                    local_z,
                    WEAK_SHORELINE_RELAX_RADIUS,
                ...
                let Some((neighbor_min, _)) =
                    direct_neighbor_terrain_range(&hydrology.columns, local_x, local_z)
                else {
                    continue;
                };

                if column.terrain_height - neighbor_min < WEAK_SHORELINE_MIN_RELIEF_BLOCKS {
                    continue;
                }

                let allowed_height = water_height
                    + WEAK_SHORELINE_BASE_FREEBOARD
                    + (water_distance.saturating_sub(1) as f32) * WEAK_SHORELINE_DISTANCE_RISE
                    + WEAK_SHORELINE_MIN_EXCESS_BLOCKS;
                let excess = column.terrain_height - allowed_height;
                if excess > 0.0 {
                    high_shoreline_columns += 1;
        ...
        assert_eq!(
            high_shoreline_columns, 0,
            "target chunk still has {high_shoreline_columns} high weak shoreline columns; max excess={max_excess:.3}"
        );
    }
    ...
    fn nearest_visible_water_neighbor_height(
        ...
        for offset_z in -1..=1 {
            for offset_x in -1..=1 {
                ...
                if let Some(water_height) =
                    hydrology.columns[column_index(neighbor_x, neighbor_z)].water_surface_height
                {
