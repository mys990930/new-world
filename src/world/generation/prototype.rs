...
use super::{
    ChunkCorridorWindow, ChunkGenerationInputs, ChunkRealizationFieldPatch, RealizationSample,
    ...
    for local_z in 0..CHUNK_EDGE_I32 {
        for local_x in 0..CHUNK_EDGE_I32 {
            ...
            columns.push(PrototypeColumn {
                base_height,
                relief_budget,
                material_support: realization.material_support,
            });
        }
    }
...
fn blended_base_height(
    ...
    let inland_signal =
        (field.continent_core_factor * 0.58 + field.inlandness * 0.42).clamp(0.0, 1.0);
    let wet_signal =
    ...
    let macro_base = macro_elevation_to_world_y(field.macro_elevation)
        + field.macro_elevation * params.macro_height_bonus
        - 6.0;
    let macro_shape = inland_signal * params.inland_lift + arid_signal * params.arid_lift
        - wet_signal * params.wet_flatten;
...
pub(super) fn blended_basis_parameters(region_samples: &[RegionSampleWeight]) -> BasisParameters {
...
pub(super) fn basis_parameters_to_realization_sample(params: BasisParameters) -> RealizationSample {
    RealizationSample {
        ...
        material_support: super::RealizationMaterialSupport::default(),
    }
}
...
fn sample_structure_basis(
    ...
    for segment in structure.mountain_chains().segments() {
        ...
        let half_width_blocks =
            (segment.half_width_cells.max(0.6) * ATLAS_CELL_BLOCK_SPAN * 0.85).max(24.0);
        let selection_radius = half_width_blocks * 3.2 + 96.0;

        if !segment_bounds_overlap_point(
            world_segment.start,
            world_segment.end,
            selection_radius,
            point,
        ...
        let core =
            smootherstep01(1.0 - projection.distance_blocks / (half_width_blocks * 0.95 + 18.0))
                * strength;
        let shoulder =
        ...
        sample.ridge_core_influence = soft_union(sample.ridge_core_influence, core.clamp(0.0, 1.0));
        ...
        if matches!(segment.scale, MountainChainScale::Major) {
            sample.major_ridge_influence = sample
                .major_ridge_influence
                .max(segment_strength.clamp(0.0, 1.0));
        }
        ...
        if segment_strength > f32::EPSILON {
            ...
            heading_axis_accum_x +=
                (tangent_x * tangent_x - tangent_z * tangent_z) * segment_strength;
    ...
    if heading_center_weight > f32::EPSILON {
        ...
        let axis_coherence =
            (heading_axis_len_sq.sqrt() * inv_heading_center_weight).clamp(0.0, 1.0);
...
fn ridge_basis(field: AtlasCell, structure: StructureBasisSample, params: BasisParameters) -> f32 {
...
fn basin_basis(field: AtlasCell, params: BasisParameters) -> f32 {
    let basin_signal =
        smootherstep01((field.basinness * 0.72 + field.lake_potential * 0.28).clamp(0.0, 1.0));
...
fn detail_basis(
    ...
    let (warped_world_x, warped_world_z) = seedless_domain_warp(
        world_x,
        world_z,
        184.0,
        14.0,
    ...
    let low_noise = seedless_value_fbm(
        warped_world_x,
        warped_world_z,
        164.0,
        3,
    ...
    let mid_noise = seedless_value_fbm(
        warped_along,
        warped_across,
        46.0,
        2,
    ...
    let flat_noise = seedless_value_fbm(
        warped_world_x,
        warped_world_z,
        20.0,
        2,
    ...
    let dune_wave = seedless_ridged_fbm(
        warped_along * 0.72,
        warped_across * 1.18,
        68.0,
        3,
        2.12,
        0.55,
        DETAIL_SALT_DUNE_FREQ,
    );
    let ridge_noise_gate =
        (0.35 + structure.ridge_shoulder_influence * 0.45 + field.ruggedness * 0.20)
            .clamp(0.0, 1.0);
...
fn corridor_adjustment_for_column(
    ...
    for branch in branch_responses {
        ...
        adjustment.strongest_influence = adjustment
            .strongest_influence
            .max(branch.response.strongest_influence);
    }
...
fn merge_branch_response(
    current: CorridorAdjustment,
    candidate: CorridorAdjustment,
) -> CorridorAdjustment {
    ...
    CorridorAdjustment {
        height_delta: (current.height_delta * current_weight
            + candidate.height_delta * candidate_weight)
            / total_weight,
...
fn single_corridor_adjustment(
    ...
    let projection = sample_corridor_axis(corridor, local_x, local_z);
    ...
    let basin_drop =
        basin_influence * mode_weights.basin_outlet * (policy.basin_depth + field.basinness * 4.0);
    ...
    let shoulder_influence = (floodplain_influence.max(basin_influence)
        * (1.0 - valley_influence * 0.82))
        .clamp(0.0, 1.0);
...
fn project_point_onto_segment(
    ...
    let t =
        (((point.0 - start.0) * seg_x + (point.1 - start.1) * seg_z) / length_sq).clamp(0.0, 1.0);
...
mod tests {
    ...
    use crate::world::generation::{
        ...
        empty_chunk_corridor_window, prepare_chunk_generation_inputs,
    };
    use crate::world::meta::WorldMeta;

    #[test]
    #[ignore = "slow current generation prototype pipeline smoke test"]
    fn base_heightfield_is_deterministic_and_emits_a_full_grid() {
        ...
        assert_eq!(
            a.columns.len(),
            (CHUNK_EDGE_I32 as usize) * (CHUNK_EDGE_I32 as usize)
        );
        assert!(
            a.columns
                .iter()
    ...
    #[test]
    #[ignore = "slow current generation seam regression test"]
    ...
    #[test]
    #[ignore = "slow current generation reported terrain regression scan"]
    ...
    fn merge_branch_response_softly_blends_similar_segment_responses() {
        ...
        assert!(
            merged.height_delta > current.height_delta
                && merged.height_delta < candidate.height_delta,
    ...
    #[test]
    #[ignore = "slow current generation wide terrain regression scan"]
    ...
    #[test]
    #[ignore = "slow generation terrain regression scan"]
    ...
    fn chunk_with_corridor_window(
        meta: &WorldMeta,
    ) -> (ChunkCoord, ChunkGenerationInputs, ChunkCorridorWindow) {
        let candidates = [
            ChunkCoord(4, 0, -3),
            ChunkCoord(0, 0, 0),
            ChunkCoord(15, 0, 15),
    ...
    fn sampled_world_height(
        ...
        let prototype = cache
            .entry(chunk)
            .or_insert_with(|| build_prototype(chunk, meta));
    ...
    fn scan_reported_wall_area(meta: &WorldMeta) -> ReportedWallScan {
        ...
        for world_z in min_world_z..=max_world_z {
            for world_x in min_world_x..=max_world_x {
                ...
                heights[dz * width + dx] = sampled_world_height(world_x, world_z, meta, &mut cache);
            }
        }
    ...
    fn assert_shared_edge_is_gradual(
        ...
        for row in 0..CHUNK_EDGE_I32 as usize {
            let left_last = left.columns
                [row * CHUNK_EDGE_I32 as usize + (CHUNK_EDGE_I32 as usize - 1)]
                .base_height;
            let left_prev = left.columns
                [row * CHUNK_EDGE_I32 as usize + (CHUNK_EDGE_I32 as usize - 2)]
                .base_height;
    ...
    fn dump_world_sample_details(world_x: i32, world_z: i32, meta: &WorldMeta) {
        ...
        let structure_sample =
            sample_structure_basis(&inputs.atlas_structure, sample_world_x, sample_world_z);
        let base_before_corridor = blended_base_height(
            params,
            field,
            structure_sample,
            sample_world_x,
