...
- player voxel cubes draw before the terrain fade pass so faded terrain can still visually cover the avatar when it sits between the camera and player
...
- Player-occluding terrain uses a bounded block-coordinate uniform to enable the effect and a player-centered vignette to shape it: opaque terrain discards the vignette region first, dynamic cubes draw, then nearby chunk terrain is redrawn through an alpha-blended fade pass so the same terrain can softly cover player voxels.
