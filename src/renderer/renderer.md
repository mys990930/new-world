...
- Terrain blocks flagged by the app as covering the local player enable a player-centered vignette: matching terrain is cut out of the opaque terrain pass and redrawn in a bounded alpha-blended terrain fade pass before dynamic player cubes, preserving existing chunk meshes while revealing the avatar without tinting the player voxels.
