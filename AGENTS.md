...
- 구현 작업은 가능한 한 기존 컨텍스트를 가진 서브에이전트 세션을 재사용한다. 매 작업마다 새 세션을 spawn하지 않는다.
- context가 이어지는 작업이고 기존 서브에이전트 세션이 그 context를 가지고 있다면, 새로 spawn하지 않고 기존 세션을 재활용한다.
- 새 에이전트는 기존 세션이 없거나, 범위가 독립적이거나, 기존 세션을 재사용하기 어려운 경우에만 만든다.
- 사용자가 명시적으로 요청하지 않는 한 main/subagent 모두 preview 이미지 생성에 시간을 쓰지 않는다. preview는 사용자가 직접 뽑아본다.
- preview 생성은 생략하더라도 테스트, 컴파일, 정적 검증은 계속 수행한다.
